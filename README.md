...
The data is reshaped in the `buildPayload` function of the Google Script [`app_script`](https://github.com/RossiLorenzo/Exiles-Randomization/blob/main/app_script.gs).

### Step3: API Call to Python code
The Python code contained in [`app.py`](https://github.com/RossiLorenzo/Exiles-Randomization/blob/main/app.py) is then triggered by the `callSolver` function of the [`app_script`](https://github.com/RossiLorenzo/Exiles-Randomization/blob/main/app_script.gs). This code maximizes the solution `ortools`.
...
The `writeAssignedTeams` function of the [`app_script`](https://github.com/RossiLorenzo/Exiles-Randomization/blob/main/app_script.gs) formats the results in tabular format and writes the raw results into the (hidden) `Assigned_Teams_Raw` tab of the [Google Sheet](https://docs.google.com/spreadsheets/d/1h5XDZbBgbXeeHlfMRaI8xbgHBjp4n94oiH3WDPw23Aw/edit?usp=sharing).

### Step5: Visualize final results
Finally, all the data is pulled back together and formatted nicely into the final view in the `Entry_List` tab of the Google Sheet.
